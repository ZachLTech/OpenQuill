    ...
    <div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl sm:max-w-7xl mx-auto">
            ...
            <div v-else class="flex flex-col items-center">
                ...
                <!-- New Instance State -->
                <div v-if="blogs?.length <= 0 || blogs == null && !instanceInitialized" class="text-center py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
                ...
                <!-- No Blogs State -->
                <div v-else-if="blogs?.length <= 0 || blogs == null && instanceInitialized" class="text-center py-12">
                ...
                <div v-else class="space-y-12 w-full px-1 sm:px-0 sm:max-w-6xl">
                    ...
                    <div v-if="postsLoading" class="flex justify-center items-center min-h-[400px]">
                        <div class="animate-spin rounded-full h-12 w-12 border-4 border-gray-800 border-t-primary"></div>
                    </div>
                    <div v-else-if="recentPosts && recentPosts.length > 0">
                        <h2 class="text-2xl font-bold text-text mb-6">Recent Posts</h2>
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            >
                                ...
                                <p class="text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-2">{{ post.summary || 'No Summary Provided' }}</p>
                                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span>{{ new Date(post.createdAt).toLocaleDateString() }}</span>
                                    <div v-if="!post.owner.website" class="flex items-center" @click.stop>
                                        <svg class="w-4 h-4 mr-1 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <span>{{ post.owner.name }}</span>
                                    </div>
                                    <NuxtLink v-if="post.owner.website" :to="post.owner.website" target="_blank" class="flex items-center" @click.stop>
                                        <svg class="w-4 h-4 mr-1 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <span>{{ post.owner.name }}</span>
                    ...
                    <!-- Blogs Section -->
                    <h2 class="text-2xl font-bold text-text mb-6">Available Blogs</h2>
                    <div class="space-y-6">
                        <NuxtLink v-for="blog in blogs" :key="blog.id" :to="`/${blog.title}`" class="block max-w-6xl h-auto min-h-[250px] mx-auto bg-secondary bg-opacity-5 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                            <div class="w-full sm:max-h-[250px] flex flex-col sm:flex-row p-6">
                                <div v-if="blog.imageURL" class="flex-shrink-0 h-[200px] aspect-square object-cover mb-4 sm:mb-0">
                                    <img :src="blog.imageURL" :alt="blog.title" class="w-full h-full object-cover rounded-lg">
                                </div>
                                <div v-else-if="!blog.imageURL" class="flex-shrink-0 h-[200px] aspect-square object-cover mb-4 sm:mb-0">
                                    <div class="w-full h-full rounded-lg border-2 border-dashed border-secondary border-opacity-15 flex items-center justify-center">
                                        <svg class="w-16 h-16 text-secondary text-opacity-15" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                        </svg>
                                    </div>
                                </div>
                                <div class="flex-grow flex flex-col items-center sm:items-baseline justify-between sm:ml-6">
                                    <h3 class="text-xl font-semibold text-text mb-2">{{ blog.title }}</h3>
                                    <p class="text-gray-600 h-[60%] dark:text-gray-300 mb-4 line-clamp-3">{{ blog.description || 'No Description Provided.' }}</p>
                                    <div class="flex items-center space-x-2 sm:space-x-4 text-sm text-gray-500 dark:text-gray-400">
                                        <div v-if="!blog.owner.website" class="flex items-center" @click.stop>
                                            <svg class="w-4 h-4 mr-1 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                            </svg>
                                            <span>{{ blog.owner.name }}</span>
                                        </div>
                                        <NuxtLink v-if="blog.owner.website" :to="blog.owner.website" target="_blank" class="flex items-center" @click.stop>
                                            <svg class="w-4 h-4 mr-1 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                            </svg>
                                            <span>{{ blog.owner.name }}</span>
                                        ...
                                        <span class="flex items-center">
                                            ...
                                            {{ blog.posts.length || 0 }} {{ blog.posts.length == 1 ? 'post' : 'posts' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div v-if="blog.tags.length > 0" class="w-full flex flex-wrap gap-2 justify-center sm:justify-normal items-center px-6 pb-6">
                                <span v-for="tag in blog.tags" class="px-2 py-1 rounded text-text text-opacity-50 bg-secondary bg-opacity-5">
                                    {{ tag }}
                                </span>
                            </div>
                        </NuxtLink>
