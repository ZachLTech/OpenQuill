...
async function deleteAccount() {
	...
	if (currentUser.value?.frozen) {
		error.value =
			"You can't delete your account while it's frozen.";
	...
	if (wantsToDeleteAccount && !currentUser.value?.admin) {
		...
		error.value =
			"You cannot delete your account if you're the only admin.";
		wantsToDeleteAccount.value = false;
		loading.value = false;
	}
}
	...
	<div class="min-h-screen bg-bg py-8 px-4 sm:px-6 lg:px-8">
		<div class="max-w-4xl mx-auto">
			...
			>
				...
				/>
				<label
					class="flex items-center text-sm font-medium text-secondary opacity-70 mb-2"
					>Profile Image
					<button
