<script setup lang="ts">
    ...
    onMounted(async () => {
        try {
            ...
            totalPublishedPages.value = Math.ceil((await $fetch('/api/blog/posts/publishedLength', {
                method: 'POST',
                body: {
            ...
            if (status.value == 'authenticated' && currentSessionUser.value && currentSessionUser.value.id == blog.value.ownerId) {
                totalUnpublishedPages.value = Math.ceil((await $fetch('/api/blog/posts/unpublishedLength', {
                    method: 'POST',
                    body: {
                ...
                const unpublishedPostsData = await $fetch('/api/blog/posts/getUnpublishedPreviews', {
                    method: 'POST',
                    body: {
    ...
    async function confirmDelete() {
        try {
            ...
            const unpublishedIndex = unpublishedPosts.value.findIndex((p: any) => p.id === postToDelete.value.id);
            ...
            const publishedIndex = publishedPosts.value.findIndex((p: any) => p.id === postToDelete.value.id);
            if (publishedIndex > -1) {
...
<template>
    ...
    <!-- Error State -->
    <div v-else-if="error" class="p-4 w-full flex justify-center absolute">
    ...
    <div v-else-if="blog" class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
        <confirmModal
        ...
        <div class="max-w-6xl mx-auto">
            <!-- Blog Header -->
            <div class="relative w-full h-[300px] rounded-xl overflow-hidden mb-8">
                <img 
                    v-if="blog.imageURL" 
                    :src="blog.imageURL" 
                    :alt="blog.title"
            ...
            <!-- Author Info -->
            <div class="flex items-center space-x-4 mb-8 p-4 bg-secondary bg-opacity-5 rounded-lg">
                <img 
            ...
            <!-- Blog Owner Controls -->
            <div v-if="status==='authenticated' && currentSessionUser?.id === blog.ownerId" 
            ...
            <!-- Post Drafts -->
            <div v-if="unpublishedPosts?.length > 0 && status == 'authenticated' && currentSessionUser?.id === blog.ownerId" class="space-y-8">
                <h2 class="text-2xl font-bold text-text mb-6">Drafts</h2>
                <div v-if="unpublishedPostsLoading" class="flex justify-center items-center min-h-[400px]">
                    <div class="animate-spin rounded-full h-12 w-12 border-4 border-gray-800 border-t-primary"></div>
                </div>
                <div v-else-if="!unpublishedPostsLoading" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <NuxtLink 
                    ...
                    >
                        ...
                        <div class="h-[35%] flex flex-col justify-between">
                            ...
                            <p class="text-gray-400 text-sm line-clamp-2 mb-4 h-[40%]">{{ post.summary || 'No Summary Available.' }}</p>
                            <div class="flex items-center justify-between text-sm text-gray-500">
                                <span>{{ new Date(post.createdAt).toLocaleDateString() }}</span>
                                <div class="flex items-center gap-2" v-if="currentSessionUser?.id === blog.ownerId">
                                    <NuxtLink 
                ...
                <!-- Posts Pagination -->
                <div v-if="totalUnpublishedPages > 1" class="flex justify-center mt-8 gap-2">
                    <button 
                    ...
                    <span class="text-text text-opacity-25 px-4 py-2">Page {{ currentUnpublishedPage }} / {{ totalUnpublishedPages }}</span>
                    <button 
                        @click="currentUnpublishedPage++"
                        :disabled="currentUnpublishedPage == totalUnpublishedPages"
                        class="px-4 py-2 rounded bg-secondary bg-opacity-10 text-text hover:bg-opacity-[0.08] transition-all disabled:opacity-50"
                    >
                        Next
                    </button>
            ...
            <div v-if="publishedPosts?.length > 0" class="space-y-8 mt-8">
                ...
                <div v-if="publishedPostsLoading" class="flex justify-center items-center min-h-[400px]">
                    <div class="animate-spin rounded-full h-12 w-12 border-4 border-gray-800 border-t-primary"></div>
                </div>
                <div v-if="!publishedPostsLoading" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <NuxtLink 
                    ...
                    >
                        ...
                        <div class="h-[35%] flex flex-col justify-between">
                            <h3 class="text-lg font-semibold text-text mb-2">{{ post.title }}</h3>
                            <p class="text-gray-400 text-sm line-clamp-2 mb-4 h-full">{{ post.summary || 'No Summary Available.' }}</p>
                            <div 
                ...
                <!-- Posts Pagination -->
                <div v-if="totalPublishedPages > 1" class="flex justify-center mt-8 gap-2">
                    <button 
                    ...
                    <span class="text-text text-opacity-25 px-4 py-2">Page {{ currentPublishedPage }} / {{ totalPublishedPages }}</span>
                    <button 
                        @click="currentPublishedPage++"
            ...
            <div v-else class="text-center py-12">
                <svg class="mx-auto h-12 w-12 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    ...
                    <path d="M16 16c0-2-1.791-4-4-4s-4 2-4 4" stroke-width="2" stroke-linecap="round" />
                </svg>
                <h3 class="mt-2 text-sm font-medium text-text">No posts yet</h3>
                <p v-if="status == 'authenticated' && currentSessionUser?.id == blog.ownerId" class="mt-1 text-sm text-gray-500">Get started by publishing your first post</p>
            </div>
        </div>
    </div>
</template>