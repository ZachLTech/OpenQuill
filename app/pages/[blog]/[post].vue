...
onMounted(async () => {
	try {
		...
		if (
			!postData.published &&
			postData.ownerId != currentSessionUser.value.id
		) {
			navigateTo(`/${route.params.blog}`);
			return;
		}
...
function replaceImageMarkers(content: string) {
	...
	images.value?.forEach((img) => {
		const srcMarkerRegex = new RegExp(
			`src="\\[image:${img.id}:"[^"]*"\\]"`,
			"g"
		);
		...
		const markdownMarkerRegex = new RegExp(
			`\\[image:${img.id}:"[^"]*"\\]`,
			"g"
		);
		processedContent = processedContent.replace(
...
<template>
	...
	<div v-if="error" class="p-4 w-full flex justify-center absolute">
		<div
			class="w-full max-w-6xl p-4 rounded-lg bg-red-500 bg-opacity-20 flex"
		>
	...
	<div v-else-if="post" class="pt-16 min-h-screen bg-bg">
		...
		<div v-else-if="post" class="max-w-6xl mx-auto px-4 sm:px-0 py-8">
			<div
				class="relative w-full h-[400px] rounded-xl overflow-hidden mb-8"
			>
				<img
					v-if="post.heroImg"
					:src="post.heroImg"
				...
				></div>
				<div
					class="absolute h-full bottom-0 left-0 flex flex-col justify-between p-8"
				>
			...
			<div
				class="flex items-center space-x-4 mb-8 p-4 pb-10 border-b border-b-secondary border-opacity-5"
			>
				<img
				...
				<div v-if="isOwner">
					<NuxtLink
						...
						:to="`/${route.params.blog}/${route.params.post}-edit`"
						>Edit Post</NuxtLink
					>
				</div>
			</div>
			...
			<div v-if="relatedPosts.length > 0" class="mt-8">
				<h2 class="text-2xl font-bold text-text mb-6">
					More Posts from
					<NuxtLink class="text-primary" :to="`/${route.params.blog}`"
						>this Blog</NuxtLink
					>
				</h2>
				<div
